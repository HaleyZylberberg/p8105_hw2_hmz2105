...
However, there are patients who developed MCI prior to being enrolled in the study and also missing participants. Will remove these participants and then redo data description.
...
There are `r nrow(MCI_both_df)` participants in both datasets. There are `r filter(MCI_full_df, amyloid == "amyloid" & is.na(mci)) |> pull(id) |> length()` participants who appear in amyloid but not MCI and `r filter(MCI_full_df, mci == "mci" & is.na(amyloid)) |> pull(id) |> length()` participants who appear in mci but not amyloid.
